...
CSV_COLUMNS_EXPERT = CSV_COLUMNS#["feature" + str(i) for i in range(1, NUM_FEATURES + 1)] + ["stage1", "value"]
...
def get_test_expert_input_fn(idx):
  return get_expert_input_fn(data_dir + "_expert" + str(idx) + ".test", batch_size=10000, num_epochs=1, shuffle=False)

def get_train_expert_input_fn(idx, batch_size=10000, num_epochs=1, shuffle=False):
  return get_expert_input_fn(data_dir + "_expert" + str(idx) + ".train", batch_size, num_epochs, shuffle)
...
def get_expert_input_fn(file_path, batch_size, num_epochs, shuffle):
  df_data = pd.read_csv(
      tf.gfile.Open(file_path),
      names=CSV_COLUMNS_EXPERT,
      skipinitialspace=True,
      engine="python",
      skiprows=1)
  labels = df_data["value"]
...
def create_expert_feature_columns():
  # Categorical features.
  return [tf.feature_column.numeric_column("feature" + str(i)) for i in range(1, NUM_FEATURES + 1)]# + [tf.feature_column.numeric_column("stage1")]
...
def create_expert_quantiles(idx, quantiles_dir):
    """Creates quantiles directory if it doesn't yet exist."""
    input_fn = get_test_expert_input_fn(idx)
    tfl.save_quantiles_for_keypoints(
        input_fn=input_fn,
        save_dir=quantiles_dir,
        feature_columns=create_expert_feature_columns(),
        num_steps=None)
...
def create_calibrated_expert_linear(feature_columns, config, quantiles_dir):
    feature_names = [fc.name for fc in feature_columns]
    hparams = tfl.CalibratedLinearHParams(
                feature_names=feature_names,
                num_keypoints=1000,
                learning_rate=0.01)
    hparams.set_feature_param("feature1", "monotonicity", 1)
    #hparams.set_feature_param("stage1", "monotonicity", 1)
    return tfl.calibrated_linear_regressor(
            feature_columns=feature_columns,
            model_dir=config.model_dir,
            config=config,
...
def create_expert_estimator(config, quantiles_dir):
    """Creates estimator for given configuration based on --model_type."""
    feature_columns = create_expert_feature_columns()
    return create_calibrated_expert_linear(feature_columns, config, quantiles_dir)
...
def main():
    ...
    for expert in range(10):
        reset()
        create_expert_quantiles(expert, quantiles_dir)
        config = tf.estimator.RunConfig().replace(model_dir=output_dir)
        cur_estimator = create_expert_estimator(config, quantiles_dir)
